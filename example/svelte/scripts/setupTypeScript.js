...
function changeJsExtensionToTs(dir) {
  ...
  for (const element of elements) {
    if (element.isDirectory()) {
      changeJsExtensionToTs(path.join(dir, element.name))
    } else if (element.name.match(/^[^_]((?!json).)*js$/)) {
      fs.renameSync(path.join(dir, element.name), path.join(dir, element.name).replace('.js', '.ts'))
    }
  }
}
